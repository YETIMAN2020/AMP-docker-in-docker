# =============================================================================
# AMP-dockerized (Podman variant)
# =============================================================================
# This is a COPY of the main `Dockerfile` that additionally bundles Podman and
# pre-applies the fixes needed to run AMP "containerized" instances (which use
# Podman) from inside this Docker container. Baking these in means you do NOT
# have to reinstall Podman or re-apply the registry fix every time the container
# is recreated/updated.
#
# What this variant adds on top of the base image:
#   * Podman (+ the packages required for rootless Podman inside a container)
#   * A default /etc/containers/registries.conf so that "short-name" images
#     (e.g. cubecoders/ampbase:debian) resolve against docker.io
#
# Runtime-only fixes (the shared-mount fix and the amp user's subuid/subgid
# ranges) are handled automatically by entrypoint/routines.sh:configure_podman.
#
# NOTE: Keep this file in sync with `Dockerfile`. Only the clearly-marked
#       "Podman variant" sections should differ from the base image.
# =============================================================================
...
ENV AMP_SUPPORT_TAGS="nosupport docker community unofficial unraid podman"
...
# ---------------------------------------------------------------------------
# Podman variant: install Podman and the packages needed to run it (rootless)
# from inside this container. AMP uses Podman for "containerized" instances.
# ---------------------------------------------------------------------------
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
    podman \
...
# ---------------------------------------------------------------------------
# Podman variant: registry fix. Clean Podman installs don't know where to look
# for "short-name" images (like cubecoders/ampbase:debian) because they lack a
# default registry configuration. Baking this in means the fix survives every
# container rebuild. (entrypoint/routines.sh re-applies it too, idempotently.)
# ---------------------------------------------------------------------------
